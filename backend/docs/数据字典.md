...
| 项目 | 实测值 |
|---|---|
| 快照时间 | `2026-07-17 13:06:51`（Asia/Shanghai） |
...
|---|---:|---|
| `app_user` | 1 | 用户主数据 |
| `app_user_current_student` | 1 | 当前学生状态 |
| `app_user_student` | 1 | 用户学生绑定 |
| `assistance_type` | 7 | 辅助方式字典 |
| `behavior_function_type` | 0 | 行为功能字典，待提供 |
...
| 2 | `avatar` | `varchar(500)` | 是 | `NULL` | - | 头像地址 |
| 3 | `name` | `varchar(64)` | 否 | 无 | - | 用户姓名 |
| 4 | `school` | `varchar(128)` | 是 | `NULL` | - | 所属学校 |
| 5 | `position` | `varchar(64)` | 是 | `NULL` | - | 职位或教师类型展示文本 |
| 6 | `role` | `enum('RESOURCE_TEACHER','SHADOW_TEACHER','PARENT')` | 否 | 无 | - | 资源教师、影子老师或家长 |
...
| 2 | `name` | `varchar(64)` | 否 | 无 | - | 学生姓名 |
| 3 | `age` | `tinyint unsigned` | 是 | `NULL` | - | 年龄 |
| 4 | `class_name` | `varchar(64)` | 是 | `NULL` | - | 班级名称 |
| 5 | `disability_type` | `varchar(64)` | 是 | `NULL` | - | 障碍类型或支持类别 |
| 6 | `support_goal` | `varchar(255)` | 是 | `NULL` | - | 学生总体支持目标 |
| 7 | `remark` | `varchar(500)` | 是 | `NULL` | - | 学生备注 |
...
| 3 | `content` | `varchar(500)` | 否 | 无 | - | 该辅助方式的具体内容，去除首尾空格后不能为空 |
...
| `behavior_record_assistance` | `chk_behavior_record_assistance_content` | `CHAR_LENGTH(TRIM(content)) > 0` |
...
|---:|---|---|---|
| 1 | `VISUAL_PROMPT` | 视觉提示 | `INTERNAL_STIMULUS` |
| 2 | `VERBAL_PROMPT` | 语言提示 | `INTERNAL_STIMULUS` |
| 3 | `ACTION_PROMPT` | 动作提示 | `INTERNAL_STIMULUS` |
| 4 | `DEMONSTRATION` | 示范 | `EXTERNAL_STIMULUS` |
| 5 | `PHYSICAL_ASSISTANCE` | 身体辅助 | `EXTERNAL_STIMULUS` |
| 6 | `REINFORCEMENT` | 强化 | `EXTERNAL_STIMULUS` |
| 7 | `ASSISTIVE_DEVICE` | 辅助设备 | `EXTERNAL_STIMULUS` |

### 8.8 行为环节和行为功能

- `behavior_stage_type`：当前 0 行，尚无正式字典值。
- `behavior_function_type`：当前 0 行，尚无正式字典值。
...
- 数据库结构与版本 `2.1.0` 一致：18 张表、79 个字段、165 项标准训练目标。
- 所有表均有主键；业务关系由 20 个外键约束保护。
- `app_user_current_student` 通过联合外键保证当前学生一定已经与用户绑定。
- 行为次数不保存独立频数字段，使用 `behavior_record` 行数统计。
- `behavior_stage_type` 和 `behavior_function_type` 当前为空，这是待业务提供的数据，不是迁移遗漏。
