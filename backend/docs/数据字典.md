...
| 项目 | 实测值 |
|---|---|
| 快照时间 | `2026-07-25 12:01:02`（Asia/Shanghai） |
...
erDiagram
    ...
    behavior_status_type ||--o{ behavior_record : "完成状态"
    behavior_record ||--o{ behavior_record_assistance : "使用辅助"
    assistance_type ||--o{ behavior_record_assistance : "辅助类型"
    behavior_record ||--o{ behavior_record_abc_tag : "选择 A/C 标签"
    abc_tag_option ||--o{ behavior_record_abc_tag : "被选择"
    abc_tag_group ||--o{ abc_tag_option : "包含"
    behavior_record ||--o{ behavior_record_catalog_selection : "选择目录状态"
    behavior_catalog_option ||--o{ behavior_record_catalog_selection : "被选择"
    ...
    environment_type ||--o{ behavior_environment_config : "推荐环境"
    behavior_type ||--o{ behavior_environment_config : "环境行为"
    behavior_module ||--o{ behavior_catalog_metadata : "包含"
    behavior_type ||--|| behavior_catalog_metadata : "目录元数据"
    behavior_module ||--o{ behavior_group : "前端分组"
    behavior_group ||--o{ behavior_group_status_option : "状态栏"
    behavior_group ||--o{ behavior_group_item : "包含"
    behavior_type ||--o{ behavior_group_item : "参与分组"
    behavior_type ||--o{ behavior_catalog_option : "子行为与状态"
    behavior_catalog_option ||--o{ behavior_catalog_option : "父子状态"
    behavior_type ||--o{ behavior_training_goal : "关联训练目标"
    training_goal ||--o{ behavior_training_goal : "被关联"
    ...
    student ||--o{ daily_evaluation : "每日评价"
```
...
| `abc_tag_group` | 11 | A/C 快捷标签分组 |
| `abc_tag_option` | 26 | A/C 快捷标签选项 |
| `assistance_type` | 9 | 辅助方式字典 |
| `behavior_function_type` | 5 | 行为功能字典 |
| `behavior_status_type` | 3 | 行为完成状态字典 |
| `behavior_module` | 9 | 标准行为模块 |
| `behavior_catalog_metadata` | 110 | 主行为目录元数据 |
| `behavior_catalog_option` | 934 | 子行为和表现/状态选项 |
| `behavior_group` | 28 | 前端推荐分组 |
| `behavior_group_status_option` | 3 | 入班准备状态栏：未完成、辅助、独立 |
| `behavior_group_item` | 95 | 分组行为明细 |
| `behavior_environment_config` | 229 | 环境与主行为推荐关系 |
| `behavior_training_goal` | 165 | 主行为与训练目标关系 |
| `behavior_record` | 0 | 行为发生记录 |
| `behavior_record_abc_tag` | 0 | 行为记录所选 A/C 快捷标签 |
| `behavior_record_assistance` | 0 | 行为辅助明细 |
| `behavior_record_catalog_selection` | 0 | 详细记录所选子行为/状态 |
| `behavior_stage_type` | 0 | 行为环节字典，待提供 |
| `behavior_type` | 110 | 标准主行为 `B001-B110` |
| `class_record` | 1 | 随班记录 |
| `course_behavior_config` | 406 | 课程与主行为推荐关系 |
| `course_type` | 16 | 课程字典 |
| `daily_evaluation` | 0 | 学生每日六维评价 |
| `environment_type` | 7 | 环境字典 |
| `schema_migration` | 10 | 迁移版本 |
| `student` | 6 | 学生主数据 |
...
当前固定为 `ATTENTION`、`TANGIBLE`、`ESCAPE`、`SENSORY`、`OTHER` 5 项。
...
### 4.10.1 `behavior_status_type` - 行为完成状态字典

当前固定为未完成、辅助完成、独立完成 3 项。快速记录初始为空；详细保存时显式选择优先，未显式选择但勾选辅助方式时自动写入 `ASSISTED`。

| 序号 | 字段 | 类型 | 空 | 默认值 | 键 | 说明 |
|---:|---|---|---|---|---|---|
| 1 | `code` | `varchar(32)` | 否 | 无 | PRI | 状态稳定代码 |
| 2 | `label` | `varchar(64)` | 否 | 无 | - | 中文名称 |
| 3 | `display_order` | `tinyint unsigned` | 否 | 无 | UNI | 唯一展示顺序 |
...
### 4.11.1 `abc_tag_group` - ABC 快捷标签分组

联合主键：`(dimension, code)`。A、C 的组编码允许相同但按维度隔离；同一维度内展示顺序唯一。

| 序号 | 字段 | 类型 | 空 | 默认值 | 键 | 说明 |
|---:|---|---|---|---|---|---|
| 1 | `dimension` | `enum('ANTECEDENT','CONSEQUENCE')` | 否 | 无 | PRI | `ANTECEDENT` 为 A，`CONSEQUENCE` 为 C |
| 2 | `code` | `varchar(64)` | 否 | 无 | PRI | 大类稳定编码 |
| 3 | `label` | `varchar(64)` | 否 | 无 | - | 大类中文名称 |
| 4 | `display_order` | `tinyint unsigned` | 否 | 无 | UNI | 维度内展示顺序 |

### 4.11.2 `abc_tag_option` - ABC 快捷标签选项

联合主键：`(dimension, code)`；联合外键 `(dimension, group_code)` 保证标签只能归入同一维度的大类。

| 序号 | 字段 | 类型 | 空 | 默认值 | 键 | 说明 |
|---:|---|---|---|---|---|---|
| 1 | `dimension` | `enum('ANTECEDENT','CONSEQUENCE')` | 否 | 无 | PRI | A 或 C |
| 2 | `code` | `varchar(64)` | 否 | 无 | PRI | 标签稳定编码 |
| 3 | `group_code` | `varchar(64)` | 否 | 无 | MUL | 所属大类编码 |
| 4 | `label` | `varchar(255)` | 否 | 无 | - | 标签中文文本 |
| 5 | `display_order` | `tinyint unsigned` | 否 | 无 | UNI | 大类内展示顺序 |
| 6 | `requires_custom_text` | `tinyint(1)` | 否 | `0` | - | 是否必须填写自定义内容；当前仅 `OTHER` 为 1 |
...
### 4.12.1 `daily_evaluation` - 学生每日评价

联合主键：`(student_id, record_date)`，每名学生每天最多一条六维评价。

| 字段 | 类型 | 说明 |
|---|---|---|
| `student_id` | `bigint unsigned` PRI/FK | 学生 ID |
| `record_date` | `date` PRI | 评价日期 |
| `emotion` | `varchar(500)` NULL | 情绪行为 |
| `adaptation` | `varchar(500)` NULL | 社会适应能力 |
| `social` | `varchar(500)` NULL | 社会交往能力 |
| `self_mgmt` | `varchar(500)` NULL | 自我行为管理 |
| `language` | `varchar(500)` NULL | 语言理解及表达 |
| `focus` | `varchar(500)` NULL | 自身/共同专注力 |
...
| 6 | `sub_behavior_code` | `varchar(64)` | 是 | `NULL` | MUL | 兼容单选的子行为编码；当前多选关系以目录选择表为准 |
| 7 | `duration_minutes` | `smallint unsigned` | 是 | `NULL` | - | 持续时间，单位分钟；非空时必须大于 0 |
| 8 | `stage_code` | `varchar(64)` | 是 | `NULL` | MUL | 行为环节代码 |
| 9 | `antecedent_text` | `varchar(1000)` | 是 | `NULL` | - | A：行为前因补充文本 |
| 10 | `behavior_description` | `varchar(1000)` | 是 | `NULL` | - | B：具体行为表现 |
| 11 | `consequence_text` | `varchar(1000)` | 是 | `NULL` | - | C：行为后果补充文本 |
| 12 | `function_code` | `varchar(64)` | 是 | `NULL` | MUL | 行为功能代码 |
| 13 | `function_other_text` | `varchar(500)` | 是 | `NULL` | - | 行为功能为 `OTHER` 时必填的自定义内容 |
| 14 | `status_code` | `varchar(32)` | 是 | `NULL` | MUL | `INCOMPLETE`、`ASSISTED`、`INDEPENDENT`；快速记录可暂为空 |
| 15 | `assistance_result_text` | `varchar(1000)` | 是 | `NULL` | - | 辅助结果自由文本 |
| 16 | `detail_saved` | `tinyint(1)` | 否 | `0` | - | 是否执行过详细记录保存；一旦为 1 不再恢复为 0 |
...
### 4.14.1 `behavior_record_abc_tag` - 行为记录 ABC 标签选择

联合主键：`(behavior_record_id, dimension, tag_code)`，同一记录在同一维度不能重复选择标签。联合外键保证 A 只能选择 A 字典、C 只能选择 C 字典。

| 序号 | 字段 | 类型 | 空 | 默认值 | 键 | 说明 |
|---:|---|---|---|---|---|---|
| 1 | `behavior_record_id` | `bigint unsigned` | 否 | 无 | PRI | 行为记录 ID |
| 2 | `dimension` | `enum('ANTECEDENT','CONSEQUENCE')` | 否 | 无 | PRI | A 或 C |
| 3 | `tag_code` | `varchar(64)` | 否 | 无 | PRI | 快捷标签编码 |
| 4 | `custom_text` | `varchar(500)` | 是 | `NULL` | - | `OTHER` 必填；非 `OTHER` 必须为空 |
...
### 4.19 `behavior_module` - 标准行为模块

| 字段 | 类型 | 说明 |
|---|---|---|
| `code` | `varchar(64)` PRI | 模块稳定编码 |
| `label` | `varchar(128)` | 模块名称 |
| `display_order` | `smallint unsigned` UNI | 模块顺序 1-9 |
| `target_start` / `target_end` | `smallint unsigned` | 文件声明的目标范围 |

### 4.20 `behavior_catalog_metadata` - 主行为目录元数据

| 字段 | 类型 | 说明 |
|---|---|---|
| `behavior_code` | `varchar(64)` PRI/FK | 主行为编码 `B001-B110` |
| `module_code` | `varchar(64)` FK | 所属模块 |
| `display_order` | `smallint unsigned` | 模块内顺序 |
| `has_sub_behaviors` | `tinyint(1)` | 是否包含标准子行为 |

### 4.21 `behavior_group` / `behavior_group_item` / `behavior_group_status_option` - 前端推荐分组

- `behavior_group`：`code` 主键，保存模块、分组名称和展示顺序。
- `behavior_group_item`：联合主键 `(group_code, behavior_code)`，保存行为在分组中的顺序。
- `behavior_group_status_option`：保存文件明确给出的分组状态栏；当前“入班准备”为未完成、辅助、独立。
- 当前共 28 个分组、95 条分组行为关系；同一主行为可以参与多个前端分组。

### 4.22 `behavior_catalog_option` - 子行为与表现/状态选项

| 字段 | 类型 | 说明 |
|---|---|---|
| `code` | `varchar(64)` PRI | 子行为使用 `Bxxx_Syy`，表现状态使用 `Bxxx_Pyy` |
| `behavior_code` | `varchar(64)` FK | 所属主行为 |
| `option_type` | `enum('SUB_BEHAVIOR','PERFORMANCE')` | 选项类型 |
| `parent_option_code` | `varchar(64)` FK/NULL | 表现状态所属子行为；主行为状态为空 |
| `label` | `varchar(255)` | 文件确认的显示文本 |
| `display_order` | `smallint unsigned` | 主行为内顺序 |
| `requires_custom_text` | `tinyint(1)` | “其它”是否强制填写自定义文本 |

### 4.23 `behavior_environment_config` - 环境行为推荐关系

联合主键 `(environment_code, behavior_code)`；当前 229 行。与 `course_behavior_config` 共同决定行为卡片，`ALL_DAY_SUMMARY` 仅跳过课程限制，仍按环境筛选。

### 4.24 `behavior_training_goal` - 主行为训练目标关系

联合主键 `(behavior_code, standard_number)`。`standard_number` 外键指向标准训练目标；`sub_behavior_code` 可选，用于精确关联子行为。当前 165 行。

### 4.25 `behavior_record_catalog_selection` - 详细记录目录选择

联合主键 `(behavior_record_id, option_code)`。`custom_text` 只允许保存 `requires_custom_text=1` 的表现状态内容；同一记录不能重复选择相同选项。

### 4.26 行为目录生成来源

- 确认版原文：`docs/source/行为条目及状态栏.md`。
- 机器可读目录：`docs/behavior-catalog.json`。
- 生成器：`scripts/generate_behavior_catalog.py`。
- 生成器校验 10 个模块、110 个主行为和完整训练目标映射，并把受控 SQL 写入 `seed.sql`。
...
| `abc_tag_group` | `PRIMARY` | 是 | `dimension, code` |
| `abc_tag_group` | `uk_abc_tag_group_order` | 是 | `dimension, display_order` |
| `abc_tag_option` | `PRIMARY` | 是 | `dimension, code` |
| `abc_tag_option` | `uk_abc_tag_option_order` | 是 | `dimension, group_code, display_order` |
...
| `behavior_status_type` | `PRIMARY` | 是 | `code` |
| `behavior_status_type` | `uk_behavior_status_type_display_order` | 是 | `display_order` |
...
| `behavior_record` | `idx_behavior_record_status` | 否 | `status_code` |
| `behavior_record` | `idx_behavior_record_sub_behavior` | 否 | `sub_behavior_code` |
| `behavior_record_abc_tag` | `PRIMARY` | 是 | `behavior_record_id, dimension, tag_code` |
| `behavior_record_abc_tag` | `idx_behavior_record_abc_tag_option` | 否 | `dimension, tag_code` |
...
| `daily_evaluation` | `PRIMARY` | 是 | `student_id, record_date` |
...
| `behavior_module` | `PRIMARY` | 是 | `code` |
| `behavior_module` | `uk_behavior_module_display_order` | 是 | `display_order` |
| `behavior_catalog_metadata` | `PRIMARY` | 是 | `behavior_code` |
| `behavior_catalog_metadata` | `uk_behavior_catalog_module_order` | 是 | `module_code, display_order` |
| `behavior_group` | `PRIMARY` | 是 | `code` |
| `behavior_group` | `uk_behavior_group_module_order` | 是 | `module_code, display_order` |
| `behavior_group_status_option` | `PRIMARY` | 是 | `code` |
| `behavior_group_status_option` | `uk_behavior_group_status_order` | 是 | `group_code, display_order` |
| `behavior_group_item` | `PRIMARY` | 是 | `group_code, behavior_code` |
| `behavior_group_item` | `idx_behavior_group_item_behavior` | 否 | `behavior_code` |
| `behavior_catalog_option` | `PRIMARY` | 是 | `code` |
| `behavior_catalog_option` | `idx_behavior_catalog_option_behavior_type` | 否 | `behavior_code, option_type, display_order` |
| `behavior_catalog_option` | `idx_behavior_catalog_option_parent` | 否 | `parent_option_code` |
| `behavior_environment_config` | `PRIMARY` | 是 | `environment_code, behavior_code` |
| `behavior_environment_config` | `idx_behavior_environment_config_behavior` | 否 | `behavior_code` |
| `behavior_record_catalog_selection` | `PRIMARY` | 是 | `behavior_record_id, option_code` |
| `behavior_record_catalog_selection` | `idx_behavior_record_catalog_option` | 否 | `option_code` |
| `behavior_training_goal` | `PRIMARY` | 是 | `behavior_code, standard_number` |
| `behavior_training_goal` | `idx_behavior_training_goal_number` | 否 | `standard_number` |
| `behavior_training_goal` | `idx_behavior_training_goal_sub_behavior` | 否 | `sub_behavior_code` |
...
| `abc_tag_option` | `fk_abc_tag_option_group` | `dimension, group_code` | `abc_tag_group` | `dimension, code` | `CASCADE` |
...
| `behavior_record` | `fk_behavior_record_status` | `status_code` | `behavior_status_type` | `code` | `NO ACTION` |
| `behavior_record` | `fk_behavior_record_stage` | `stage_code` | `behavior_stage_type` | `code` | `NO ACTION` |
| `behavior_record` | `fk_behavior_record_sub_behavior` | `sub_behavior_code` | `behavior_catalog_option` | `code` | `NO ACTION` |
| `behavior_record_abc_tag` | `fk_behavior_record_abc_tag_record` | `behavior_record_id` | `behavior_record` | `id` | `CASCADE` |
| `behavior_record_abc_tag` | `fk_behavior_record_abc_tag_option` | `dimension, tag_code` | `abc_tag_option` | `dimension, code` | `NO ACTION` |
...
| `daily_evaluation` | `fk_daily_evaluation_student` | `student_id` | `student` | `id` | `CASCADE` |
...
| `behavior_catalog_metadata` | `fk_behavior_catalog_metadata_behavior` | `behavior_code` | `behavior_type` | `code` | `CASCADE` |
| `behavior_catalog_metadata` | `fk_behavior_catalog_metadata_module` | `module_code` | `behavior_module` | `code` | `NO ACTION` |
| `behavior_group` | `fk_behavior_group_module` | `module_code` | `behavior_module` | `code` | `CASCADE` |
| `behavior_group_status_option` | `fk_behavior_group_status_group` | `group_code` | `behavior_group` | `code` | `CASCADE` |
| `behavior_group_item` | `fk_behavior_group_item_group` | `group_code` | `behavior_group` | `code` | `CASCADE` |
| `behavior_group_item` | `fk_behavior_group_item_behavior` | `behavior_code` | `behavior_type` | `code` | `CASCADE` |
| `behavior_catalog_option` | `fk_behavior_catalog_option_behavior` | `behavior_code` | `behavior_type` | `code` | `CASCADE` |
| `behavior_catalog_option` | `fk_behavior_catalog_option_parent` | `parent_option_code` | `behavior_catalog_option` | `code` | `CASCADE` |
| `behavior_environment_config` | `fk_behavior_environment_config_environment` | `environment_code` | `environment_type` | `code` | `NO ACTION` |
| `behavior_environment_config` | `fk_behavior_environment_config_behavior` | `behavior_code` | `behavior_type` | `code` | `CASCADE` |
| `behavior_record_catalog_selection` | `fk_behavior_record_catalog_selection_record` | `behavior_record_id` | `behavior_record` | `id` | `CASCADE` |
| `behavior_record_catalog_selection` | `fk_behavior_record_catalog_selection_option` | `option_code` | `behavior_catalog_option` | `code` | `NO ACTION` |
| `behavior_training_goal` | `fk_behavior_training_goal_behavior` | `behavior_code` | `behavior_type` | `code` | `CASCADE` |
| `behavior_training_goal` | `fk_behavior_training_goal_goal` | `standard_number` | `training_goal` | `standard_number` | `NO ACTION` |
| `behavior_training_goal` | `fk_behavior_training_goal_sub_behavior` | `sub_behavior_code` | `behavior_catalog_option` | `code` | `CASCADE` |
...
| `behavior_record` | `chk_behavior_record_function_other` | `OTHER` 必须填写 `function_other_text`；其他功能必须为空 |
| `behavior_record_abc_tag` | `chk_behavior_record_abc_tag_custom` | `OTHER` 必须填写 `custom_text`；其他标签必须为空 |
...
课程共 16 项。“劳动”“综合”“地方”不设独立字典项，统一使用 `OTHER`，具体名称可保存到 `class_record.course_other_description`。
...
主行为稳定编码为 `B001-B110`，编码顺序严格等于确认文件中的模块顺序和行为顺序。完整 110 项及其子行为/状态见 `behavior-catalog.json`，模块分布如下：

| 顺序 | 模块 code | 模块名称 | 主行为数 |
|---:|---|---|---:|
| 1 | `SCHOOL_CLASS_AWARENESS` | 学校/班级意识 | 10 |
| 2 | `SCHOOL_ENTRY` | 入校常识 | 9 |
| 3 | `SCHOOL_LEAVING` | 离校常规 | 8 |
| 4 | `SPORTS` | 运动 | 15 |
| 5 | `EXERCISES` | 做操 | 9 |
| 6 | `STAIRS` | 上下楼梯 | 11 |
| 7 | `QUEUE` | 排队 | 9 |
| 8 | `DINING` | 用餐 | 12 |
| 9 | `BREAK_TIME` | 课间休息 | 15 |
| 10 | `GROUP_CLASS` | 集体课 | 12 |
...
- `behavior_function_type`：固定使用以下 5 项。
...
`behavior_status_type` 固定使用以下 3 项：
...
`dimension=ANTECEDENT`，共 5 个大类、14 个标签。接口按大类和标签的 `display_order` 返回。

| 大类 code | 大类名称 | 标签 code | 标签名称 | 自定义 |
|---|---|---|---|---|
| `TASK_INSTRUCTION` | 任务与指令类 | `TASK_DIFFICULT` | 任务难度过高/无法理解 | 否 |
| `TASK_INSTRUCTION` | 任务与指令类 | `TASK_NEW` | 布置新任务/提出新要求 | 否 |
| `TASK_INSTRUCTION` | 任务与指令类 | `TASK_URGED` | 被催促完成任务 | 否 |
| `ENVIRONMENT_SENSORY` | 环境与感官类 | `ENV_NOISY` | 环境嘈杂/刺激过载 | 否 |
| `ENVIRONMENT_SENSORY` | 环境与感官类 | `TRANSITION` | 场景转换/换课件/换教室/换老师 | 否 |
| `ENVIRONMENT_SENSORY` | 环境与感官类 | `UNSTRUCTURED` | 非结构化时间（如课间自由活动） | 否 |
| `SOCIAL_INTERACTION` | 社交与互动类 | `PEER_CONFLICT` | 同伴干扰/冲突/嘲笑 | 否 |
| `SOCIAL_INTERACTION` | 社交与互动类 | `REQUEST_DENIED` | 需求或提议被拒绝 | 否 |
| `SOCIAL_INTERACTION` | 社交与互动类 | `ATTENTION_SHIFT` | 教师或家长关注其他 | 否 |
| `ITEM_ACTIVITY` | 物品与活动类 | `ACTIVITY_STOPPED` | 被要求停止喜爱的活动 | 否 |
| `ITEM_ACTIVITY` | 物品与活动类 | `ITEM_DENIED` | 拿不到喜爱的物品 | 否 |
| `PHYSICAL_OTHER` | 生理与其它 | `PHYSICAL_DISCOMFORT` | 疑似身体不适/疲劳 | 否 |
| `PHYSICAL_OTHER` | 生理与其它 | `NO_OBVIOUS_TRIGGER` | 无明显外在诱因 | 否 |
| `PHYSICAL_OTHER` | 生理与其它 | `OTHER` | 其他 | 是，必填 |

### 8.8.2 C：行为后果快捷标签

`dimension=CONSEQUENCE`，共 6 个大类、12 个标签。

| 大类 code | 大类名称 | 标签 code | 标签名称 | 自定义 |
|---|---|---|---|---|
| `TEACHER_RESPONSE` | 教师反馈类 | `VERBAL_PROMPT` | 口头提醒/讲道理/批评 | 否 |
| `TEACHER_RESPONSE` | 教师反馈类 | `VERBAL_SOOTHING` | 语言安抚/情绪疏导 | 否 |
| `TEACHER_RESPONSE` | 教师反馈类 | `PLANNED_IGNORING` | 战术性忽略/冷处理 | 否 |
| `TASK_ENVIRONMENT_ADJUSTMENT` | 任务与环境调整类 | `TASK_PAUSED` | 任务暂停/延期 | 否 |
| `TASK_ENVIRONMENT_ADJUSTMENT` | 任务与环境调整类 | `TASK_SIMPLIFIED` | 降低任务难度/提供替代任务 | 否 |
| `TASK_ENVIRONMENT_ADJUSTMENT` | 任务与环境调整类 | `REMOVED_FROM_ENV` | 被带离当前环境（如离开教室） | 否 |
| `PEER_RESPONSE` | 同伴反应类 | `PEER_ATTENTION` | 同伴注视/议论/围观 | 否 |
| `PEER_RESPONSE` | 同伴反应类 | `PEER_YIELDED` | 同伴妥协/退让/给予物品 | 否 |
| `ASSISTANCE_INTERVENTION` | 辅助介入类 | `PHYSICAL_ASSIST` | 给予肢体辅助 | 否 |
| `ASSISTANCE_INTERVENTION` | 辅助介入类 | `VISUAL_ASSIST` | 给予视觉/手势提示 | 否 |
| `ITEM_ACCESS` | 物品获得类 | `ITEM_GIVEN` | 给予喜爱的物品或活动（作为安抚或强化） | 否 |
| `OTHER` | 其他 | `OTHER` | 其他 | 是，必填 |

前端显示颜色属于表现层，不保存到数据库；应以 `groupCode` 区分大类，不应根据中文名称反推业务 code。A/C 标签允许多选，后端拒绝跨维度 code、重复 code、未知 code 和不符合 `OTHER` 规则的自定义内容。
...
当前 406 行均为确认文件的系统推荐关系，`configured_by_user_id` 为 `NULL`。完整关系写入 `behavior-catalog.json` 的 `recommendedCourseCodes`。`ALL_DAY_SUMMARY` 不单独生成配置行，接口读取该课程时忽略课程限制并仍按环境筛选。
...
- 数据库结构与版本 `2.9.0` 一致：32 张表、145 个字段、3 名白名单教师、6 名学生、16 项课程、7 项环境、5 项行为功能、3 项行为完成状态、110 个主行为、88 个子行为、846 个表现/状态选项、11 个 ABC 标签分组、26 个 A/C 快捷标签和 165 项标准训练目标。
- 所有表均有主键；业务关系由 41 个外键约束保护。
- `app_user_current_student` 通过联合外键保证当前学生一定已经与用户绑定。
- 行为次数不保存独立频数字段，使用 `behavior_record` 行数统计。
- 110 个主行为由课程、环境和训练目标关系表约束；父子选项不存在孤儿关系。
- `behavior_stage_type` 当前为空；行为功能已固定为 5 项，行为完成状态固定为 3 项。
