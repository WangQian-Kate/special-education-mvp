...
| 项目 | 实测值 |
|---|---|
| 快照时间 | `2026-07-23 09:31:34`（Asia/Shanghai） |
| MySQL | `8.0.45 MySQL Community Server - GPL` |
| 数据库 | `special_ed_assistant` |
| 当前结构版本 | `2.7.0` |
| 已应用版本 | `2.0.0`、`2.1.0`、`2.2.0`、`2.3.0`、`2.4.0`、`2.5.0`、`2.6.0`、`2.7.0` |
| 默认字符集 | `utf8mb4` |
...
| `behavior_catalog_metadata` | 100 | 主行为目录元数据 |
| `behavior_catalog_option` | 619 | 88 个子行为和 531 个表现/状态选项 |
| `behavior_group` | 28 | 前端推荐分组 |
| `behavior_group_status_option` | 3 | 入班准备状态栏：未完成、辅助、独立 |
| `behavior_group_item` | 95 | 分组行为明细 |
| `behavior_environment_config` | 196 | 环境与主行为推荐关系 |
| `behavior_training_goal` | 155 | 主行为与训练目标 11-165 关系 |
...
| `behavior_type` | 100 | 标准主行为 `B001-B100` |
| `class_record` | 0 | 随班记录 |
| `course_behavior_config` | 368 | 课程与主行为推荐关系 |
| `course_type` | 16 | 课程字典 |
| `environment_type` | 7 | 环境字典 |
| `schema_migration` | 8 | 迁移版本 |
| `student` | 6 | 学生主数据 |
...
| 12 | `function_other_text` | `varchar(500)` | 是 | `NULL` | - | 行为功能为 `OTHER` 时必填的自定义内容 |
| 13 | `status_code` | `varchar(32)` | 是 | `NULL` | MUL | `INCOMPLETE`、`ASSISTED`、`INDEPENDENT`；快速记录可暂为空 |
| 14 | `assistance_result_text` | `varchar(1000)` | 是 | `NULL` | - | 辅助结果自由文本 |
| 15 | `detail_saved` | `tinyint(1)` | 否 | `0` | - | 是否执行过详细记录保存；一旦为 1 不再恢复为 0 |
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `behavior_code` | `varchar(64)` PRI/FK | 主行为编码 `B001-B100` |
...
联合主键 `(environment_code, behavior_code)`；当前 196 行。与 `course_behavior_config` 共同决定行为卡片，`ALL_DAY_SUMMARY` 仅跳过课程限制，仍按环境筛选。

### 4.24 `behavior_training_goal` - 主行为训练目标关系

联合主键 `(behavior_code, standard_number)`。`standard_number` 外键指向标准训练目标；`sub_behavior_code` 可选，用于精确关联子行为。当前 155 行，精确且不重复地覆盖目标 11-165。
...
- 生成器校验 9 个模块、100 个主行为、训练目标 11-165 不重不漏，并把受控 SQL 写入 `seed.sql`。
...
主行为稳定编码为 `B001-B100`，编码顺序严格等于确认文件中的模块顺序和行为顺序。完整 100 项及其子行为/状态见 `behavior-catalog.json`，模块分布如下：

| 顺序 | 模块 code | 模块名称 | 主行为数 |
|---:|---|---|---:|
| 1 | `SCHOOL_ENTRY` | 学校/班级意识、入校常识 | 9 |
| 2 | `SCHOOL_LEAVING` | 离校常规 | 8 |
| 3 | `SPORTS` | 运动 | 15 |
| 4 | `EXERCISES` | 做操 | 9 |
| 5 | `STAIRS` | 上下楼梯 | 11 |
| 6 | `QUEUE` | 排队 | 9 |
| 7 | `DINING` | 用餐 | 12 |
| 8 | `BREAK_TIME` | 课间休息 | 15 |
| 9 | `GROUP_CLASS` | 集体课 | 12 |
...
当前 368 行均为确认文件的系统推荐关系，`configured_by_user_id` 为 `NULL`。完整关系写入 `behavior-catalog.json` 的 `recommendedCourseCodes`。`ALL_DAY_SUMMARY` 不单独生成配置行，接口读取该课程时忽略课程限制并仍按环境筛选。
...
- 数据库结构与版本 `2.7.0` 一致：28 张表、122 个字段、3 名白名单教师、6 名学生、16 项课程、7 项环境、5 项行为功能、3 项行为完成状态、100 个主行为、88 个子行为、531 个表现/状态选项、28 个分组、3 个分组状态栏选项和 165 项标准训练目标。
- 所有表均有主键；业务关系由 36 个外键约束保护。
- `app_user_current_student` 通过联合外键保证当前学生一定已经与用户绑定。
- 行为次数不保存独立频数字段，使用 `behavior_record` 行数统计。
- 100 个主行为均至少关联一个课程、一个环境和一个训练目标；训练目标关联精确覆盖 11-165，父子选项不存在孤儿关系。
