...
本设计依据 2026-07-16 至 2026-07-17 已确认的软件原型和交互规则，替代此前以旧接口为基础的数据模型。V2 SQL、受保护迁移脚本和 Java 接口已经实现；正式 MySQL 已于 2026-07-17 切换到 V2.1.0。
...
这是技术表，不属于业务页面。每个已成功应用的结构版本保存一行；当前最新版本为 `2.1.0`，用于保证迁移和标准数据脚本可追踪。
...
- `backend/sql/seed.sql`：本地开发种子数据，可重复执行；包含 10 个标准分类和 165 项标准目标，不会伪造尚未提供的等级、行为环节或行为功能。
