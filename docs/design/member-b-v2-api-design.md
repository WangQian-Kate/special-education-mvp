...
前端提出的 `X-Teacher-Id: t001/t002/t003` 伪登录映射本轮暂缓。确定教师 ID 与数据库用户的对应关系后，再替换当前临时请求头。
...
V2 设计已经实施。当前 OpenAPI 版本为 0.5.0，Java、SQL 和正式数据库已同步；需将 0.5.0 重新导入现有新版 Apifox 模块。
